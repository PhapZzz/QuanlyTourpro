...
export default function AdminSuppliers() {
  ...
  const load = async () => {
    setLoading(true)
    try { const r = await supplierAPI.getAll({ search }); setData(r.data.data ?? []) }
  ...
  const cols = [
    { title:'Mã', dataIndex:'code', width:80, render:v=><span style={{color:'#888',fontSize:12}}>{v}</span> },
    { title:'Tên nhà cung cấp', dataIndex:'name', render:(v,r)=><span style={{fontWeight:500,cursor:'pointer'}} onClick={()=>openEdit(r)}>{v}</span> },
    ...
    { title:'', key:'act', width:100, render:(_,row)=>(
      <Space size={4}>
        <Button size="small" icon={<EditOutlined />} onClick={()=>openEdit(row)} />
        <Popconfirm title="Vô hiệu hóa NCC?" onConfirm={()=>supplierAPI.update(row.id,{status:'INACTIVE'}).then(load)}>
  ...
  return (
    <>
      <div style={{display:'flex',justifyContent:'space-between',marginBottom:16,gap:8}}>
        ...
        <Space>
          <Input.Search placeholder="Tìm nhà cung cấp..." style={{width:240}} onSearch={setSearch} allowClear />
      ...
      <Modal title={editing?'Chỉnh sửa NCC':'Thêm nhà cung cấp'} open={open}
        ...
        <Form form={form} layout="vertical" onFinish={onFinish}>
          ...
          <Form.Item name="type" label="Loại dịch vụ" rules={[{required:true}]}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
            ...
            <Form.Item name="contactPerson" label="Người liên hệ"><Input /></Form.Item>
          </div>
