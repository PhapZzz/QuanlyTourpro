...
// Đổi field 'children' → 'childCount' vì antd Table dùng 'children' làm reserved word cho tree data
const normalize = ({ children, ...rest }) => ({ ...rest, childCount: children ?? 0 })
...
export default function SalesBookings() {
  ...
  const load = async () => {
    setLoad(true)
    try {
      const r = await bookingAPI.getAll({ status: statusF || undefined, size: 50 })
      const content = r?.data?.data?.content ?? []
      setData(content.map(normalize))
    } catch {
      message.error('Không tải được danh sách đặt tour')
    } finally {
      setLoad(false)
    }
  }
  ...
  useEffect(() => {
    customerAPI.getAll({ size: 100 })
      .then(r => setCust((r?.data?.data?.content ?? []).map(c => ({ value: c.id, label: `${c.code} – ${c.fullName}` }))))
      .catch(() => {})

    tourAPI.getAll({ status: 'ACTIVE', size: 50 })
      .then(r => setTours((r?.data?.data?.content ?? []).map(t => ({ value: t.id, label: t.name, schedules: t.schedules ?? [] }))))
      .catch(() => {})
  }, [])
    ...
    },
    {
      ...
      render: (_, r) => (
        <div>
          ...
          <span style={{ fontSize: 11, color: '#888' }}>{r.customerPhone}</span>
        </div>
      ),
    },
    {
      ...
      render: (_, r) => (
        <div>
          <strong>{r.tourName}</strong><br />
          <span style={{ fontSize: 11, color: '#888' }}>{r.departureDate}</span>
        </div>
      ),
    },
    {
      title: 'Số người', key: 'pax',
      render: (_, r) => `${r.adults} NL${r.childCount ? ` + ${r.childCount} TE` : ''}`,
    },
    {
    ...
    },
    {
      ...
      render: (_, row) => (
        <Space size={4}>
          ...
          {row.status === 'PENDING' && (
            <Button size="small" type="primary" ghost onClick={() => updateStatus(row.id, 'CONFIRMED')}>
              Xác nhận
            </Button>
          )}
          {row.status === 'CONFIRMED' && (
            <Button size="small" style={{ color: '#1D9E75', borderColor: '#1D9E75' }} onClick={() => updateStatus(row.id, 'PAID')}>
              Đã TT
            </Button>
          )}
        </Space>
  ...
  const detailFooter = useMemo(() => {
    if (!detail) return [<Button key="close" onClick={() => setDet(null)}>Đóng</Button>]
    if (detail.status === 'CANCELLED' || detail.status === 'COMPLETED') {
      return [<Button key="close" onClick={() => setDet(null)}>Đóng</Button>]
    }
    const btns = [<Button key="close" onClick={() => setDet(null)}>Đóng</Button>]
    if (detail.status === 'PENDING') {
      btns.push(
        <Button key="confirm" type="primary" ghost
          onClick={() => { updateStatus(detail.id, 'CONFIRMED'); setDet(null) }}>
          Xác nhận
        </Button>
      )
    }
    if (detail.status === 'CONFIRMED') {
      btns.push(
        <Button key="paid" style={{ color: '#1D9E75', borderColor: '#1D9E75' }}
          onClick={() => { updateStatus(detail.id, 'PAID'); setDet(null) }}>
          Đánh dấu đã TT
        </Button>
      )
    }
    btns.push(
      <Button key="cancel" danger onClick={() => { updateStatus(detail.id, 'CANCELLED'); setDet(null) }}>
        Hủy đơn
      </Button>
    )
    return btns
    ...
    <>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16, gap: 8, flexWrap: 'wrap' }}>
        ...
        <Space>
          ...
          />
          <Button type="primary" icon={<PlusOutlined />} style={{ background: '#1D9E75' }}
      ...
      {/* Modal tạo đơn */}
      <Modal title="Lập phiếu đặt tour" open={open} width={600}
        onCancel={() => setOpen(false)} onOk={() => form.submit()} okText="Xác nhận đặt" destroyOnHidden>
        <Form form={form} layout="vertical" onFinish={onFinish}>
          <Form.Item name="customerId" label="Khách hàng" rules={[{ required: true, message: 'Chọn khách hàng' }]}>
            <Select showSearch options={customers}
          ...
          <Form.Item name="tourScheduleId" label="Ngày khởi hành" rules={[{ required: true, message: 'Chọn ngày khởi hành' }]}>
            <Select options={schedules} disabled={!schedules.length}
              placeholder={schedules.length ? 'Chọn ngày' : 'Chọn tour trước'} />
          </Form.Item>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <Form.Item name="adults" label="Số người lớn" rules={[{ required: true }]} initialValue={2}>
      ...
      {/* Modal chi tiết */}
      <Modal title={`Chi tiết đơn – ${detail?.code ?? ''}`} open={!!detail}
        onCancel={() => setDet(null)} width={580} footer={detailFooter} destroyOnHidden>
        {detail && (
          <Descriptions column={2} bordered size="small">
            ...
            <Descriptions.Item label="Số người">
              {detail.adults} NL{detail.childCount ? ` + ${detail.childCount} TE` : ''}
            </Descriptions.Item>
            <Descriptions.Item label="Tổng tiền">
              <strong style={{ color: '#1D9E75' }}>{fmt(detail.totalPrice)}</strong>
            </Descriptions.Item>
