...
export const employeeAPI = {
  ...
  getByUserId:        (id)       => api.get(`/hr/employees/user/${id}`),
  ...
  getPositionHistory: (id)       => api.get(`/hr/employees/${id}/position-history`),
}

export const salaryAPI = {
  getByMonthYear: (month, year) => api.get('/hr/salary', { params: { month, year } }),
  getByEmployee:  (empId)       => api.get(`/hr/salary/employee/${empId}`),
  calculate:      (empId, data) => api.post(`/hr/salary/${empId}/calculate`, data),
  calculateAll:   (data)        => api.post('/hr/salary/calculate-all', data),
  approve:        (id)          => api.post(`/hr/salary/${id}/approve`),
  updateBonus:    (id, data)    => api.patch(`/hr/salary/${id}/bonus`, data),
}
...
export const tourAPI = {
  ...
  getServices:     (tourId)              => api.get(`/tours/${tourId}/services`),
  addService:      (tourId, data)        => api.post(`/tours/${tourId}/services`, data),
  updateService:   (tourId, productId, data) => api.put(`/tours/${tourId}/services/${productId}`, data),
  removeService:   (tourId, productId)   => api.delete(`/tours/${tourId}/services/${productId}`),
  recalculatePrice:(tourId, margin = 30) => api.post(`/tours/${tourId}/recalculate-price`, null, { params: { margin } }),
}
